...
PATH_TO_CKPT = os.path.join(CWD_PATH, 'dojo', 'yolo', 'yolov4_smartcart', 'saved_model.pb')
print("Path to model: {}".format(PATH_TO_CKPT))

# Path to label map file
PATH_TO_LABELS = os.path.join(CWD_PATH, 'dojo', 'yolo', 'yolov4_smartcart')
...
label_map = label_map_util.load_labelmap('/home/pi/projects/smartcart-device/dojo/yolo/yolov4_smartcart/smartcart_label_map.pbtxt')
...
with detection_graph.as_default():
    ...
    with tf.gfile.GFile('/home/pi/projects/smartcart-device/dojo/yolo/yolov4_smartcart/testing/saved_model.pb', 'rb') as fid:
...
detection_graph = tf.Graph()
sess = tf.Session(graph=detection_graph)
tf.saved_model.loader.load(sess, [tf.saved_model.tag_constants.SERVING], '/home/pi/projects/smartcart-device/dojo/yolo/yolov4_smartcart')

# Define input and output tensors (i.e. data) for the object detection classifier

[print(tensor) for op in detection_graph.get_operations() for tensor in op.values()]
...
def person_detector(frame):
    ...
    # Draw counter info
    #cv2.putText(frame, 'Detection counter: ' + str(max(counter)), (10, 100), font, 0.5, (255, 255, 0), 1, cv2.LINE_AA)
...
if camera_type == 'picamera':
    ...
    for frame1 in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        frame = frame1.array
        frame = np.require(frame, requirements=['O', 'W'])
